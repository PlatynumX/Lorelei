# ROTT64 Revision 66 — Diagnostic Display Handoff Test (Darkwar)

- Keeps the boot diagnostic display/VI alive instead of calling `display_close()`
  immediately before handing control to Taradino.
- Guards the gameplay renderer's first `display_init()` so it does not tear down
  and recreate an already-active display during startup.
- No intended gameplay, rumble, sound, music, save, or control changes.
- This is a focused test for the black screen occurring immediately after the
  startup diagnostic screen.
- Public ROM filename: `rott64-darkwar-r66.z64`.

# ROTT64 Revision 65 — Dark War Legacy Warning Build Fix

- Stops treating warnings as fatal only for generated upstream Taradino engine objects.
- Uses GNU Make target-specific flags:
  `$(BUILD_DIR)/generated/rott/%.o: CFLAGS += -Wno-error`
- Native N64 platform code remains under the normal strict libdragon warning policy.
- Removes the incomplete per-warning pragma workaround from R64.
- Keeps the proven internal `rott64.z64` / `build/rott64.elf` build setup.
- Keeps full Dark War data and full `--mode full` soundtrack conversion.
- Keeps the corrected runtime `DARKWAR.WAD` startup probe.
- Final ROM filename: `rott64-darkwar-r65.z64`.

# ROTT64 Revision 64 — Dark War Build and Startup Fix

- Restores strict project-wide `-Werror`; removes all Makefile `-Wno-error` flags.
- Suppresses only GCC's `maybe-uninitialized` and `uninitialized` false positives
  inside generated legacy Taradino `.c` files.
- Fixes the Dark War N64 startup probe to require `rom://rott/DARKWAR.WAD`
  instead of the stale Shareware `HUNTBGIN.WAD` path.
- Updates host tests to validate the Dark War startup path and generated-source
  warning guard.
- Keeps the proven internal `rott64.z64` / `build/rott64.elf` libdragon target.
- Keeps the dedicated full Dark War WAD and `--mode full` soundtrack pipeline.
- Final ROM filename: `rott64-darkwar-r64.z64`.

# ROTT64 Revision 63 — Dark War Host Warning-Policy Fix

- Fixes the Dark War host-validation failure introduced by the narrow legacy
  warning demotions in R62.
- The host test now permits exactly:
  - `-Wno-error=maybe-uninitialized`
  - `-Wno-error=uninitialized`
- The test still rejects any broad `-Wno-error` usage.
- Shareware remains untouched.
- Keeps the proven R25 internal `rott64.z64` build target.
- Keeps Dark War full data and full soundtrack pipeline.
- Final ROM name: `rott64-darkwar-r63.z64`.

# ROTT64 Revision 62 — Dark War Full-Build Warning Fix

- Shareware R61 is unchanged and remains the working Shareware baseline.
- Fixes the Dark War-only N64 cross-build failure caused by legacy Taradino
  full-version code paths triggering GCC `maybe-uninitialized` / `uninitialized`
  diagnostics under global `-Werror`.
- Warnings remain enabled and visible in the build log.
- Only those two warning classes are demoted from fatal errors.
- Keeps the proven internal `rott64.z64` / `build/rott64.elf` libdragon target.
- Keeps full Dark War data and the full soundtrack pipeline.
- Final ROM name is `rott64-darkwar-r62.z64`.

# ROTT64 Revision 61 — Restore Proven Libdragon Build Target (Dark War)

- Restores the proven internal R25 libdragon ROM target: `rott64.z64`.
- Keeps `build/rott64.elf` linked from the full object list, fixing the
  `undefined reference to main` regression caused by renaming the Makefile target.
- The finished ROM is renamed only after a successful build to `rott64-darkwar-r61.z64`.
- Dark War remains a dedicated build.
- Dark War is compiled without `-DSHAREWARE=1` and uses `filesystem/rott/DARKWAR.WAD` at the runtime DATADIR root.
- Dark War full soundtrack extraction/rendering remains enabled with `--mode full`.

- R25 rumble/audio baseline remains otherwise unchanged.
