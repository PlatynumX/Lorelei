...
static int current,initialized,available,loaded;
...
static int load(void){
    ...
    /* During SaveTheGame, Taradino closes the write descriptor and reopens
       the same image to calculate its inner checksum before appending it. */
    ...
    free(s);raw_size=h.raw_size;return 1;
}
static int commit(void){
    ...
    free(s);loaded=1;return 1;
}
static int native(const char*p){
    const char*n;if(!p)return 0;n=strrchr(p,'/');n=n?n+1:p;return strcmp(n,"rottgam0.rot")==0;
}
int rott64_save_open_write(const char*p){if(!native(p)){errno=EROFS;return-1;}if(!reserve(65536))return-1;raw_size=pos=0;current=HWRITE;return current;}
int rott64_save_open_append(const char*p){if(!native(p)){errno=EROFS;return-1;}pos=raw_size;current=HAPPEND;return current;}
int rott64_save_open_read(const char*p){if(!native(p)||!load()){errno=ENOENT;return-1;}pos=0;current=HREAD;return current;}
...
int rott64_save_load_file(const char*p,void**b){void*q;if(!b||!native(p)||!load()){errno=ENOENT;return-1;}q=malloc(raw_size);if(!q){errno=ENOMEM;return-1;}memcpy(q,raw,raw_size);*b=q;return(int)raw_size;}
char *rott64_save_case_exists(const char*p){char*q;if(!native(p)||!load())return NULL;q=malloc(strlen(p)+1);if(q)strcpy(q,p);return q;}
int rott64_save_unlink(const char*p){save_header_t h;if(!native(p)){errno=EROFS;return-1;}init();if(!available){errno=ENODEV;return-1;}memset(&h,0xff,sizeof(h));raw_size=0;loaded=1;return flashram_write(&h,0,sizeof(h))==(int)sizeof(h)?0:-1;}
