...
fromloadedgame:
	...
	while (playstate == ex_stillplaying)
	{
		...
		if ((!BATTLEMODE) && (CP_CheckQuick(LastScan)))
		{
			...
			#ifdef __N64__
			/* ROTT64_R122_SKIP_REDUNDANT_SETUPSCREEN_AFTER_LOAD:
			 * LoadTheGame already rebuilt the play screen. Avoid running the
			 * normal menu-return SetupScreen(false) a second time before the
			 * existing loadedgame -> fromloadedgame transition. */
