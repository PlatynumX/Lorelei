...
void W_ReadLump(int lump, void *dest)
{
	 ...
	 * Break large synchronous WAD reads into bounded transfers so the
	 * cooperative libdragon mixer can refill the AI queue while loading. */
	c = 0;
	rott64_mixer_pump();
	while (c < l->size)
	{
		int request = l->size - c;
		int got;
		if (request > 32768)
			request = 32768;
		got = read(l->handle, (byte *)dest + c, request);
		if (got <= 0)
			break;
		c += got;
		rott64_mixer_pump();
