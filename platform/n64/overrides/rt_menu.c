...
CP_iteminfo DetailItems = { 32, 64, 4, 0, 43, DetailMenuNames, mn_largefont };
...
/* ROTT64_R96C_SD_SAVE_MENU_HELPER: return path only when save file exists. */
static char *rott64_r96c_existing_save_path(const char *path)
{
    FILE *fp;
...
void ScanForSavedGames()
{
    int which;
    boolean found = false;
    gamestorage_t *game = &rott64_save_scratch; /* ROTT64_R109_ROTTDS_SAVE_SCRATCH */

    /* ROTT64_R96C_SCAN_VALIDATES_SLOTS */
    ...
    {
        /* ROTT64_R110_MENU_HEADER_ONLY_SCAN: do not checksum the whole save while opening the menu. */
...
{
#if defined(__N64__)
    /* ROTT64_R88_SD_GUARD_CP_LoadGame */
...
/* ROTT64_R96C_START_INGAME_SAVE_CONTEXT: saves require an active gameplay context, not title/menu state. */
static int rott64_r96c_can_save_in_current_context(void)
{
...
{
    /* ROTT64_R96C_SAVE_CONTEXT_GUARD */
    if (!rott64_r96c_can_save_in_current_context())
    {
        printf("ROTT64 r96c: Save rejected outside active game context\n");
        return 0;
    }

#if defined(__N64__)
    /* ROTT64_R88_SD_GUARD_CP_SaveGame */
    extern int rott64_n64_sd_saves_ready(void);
    if (!rott64_n64_sd_saves_ready())
        return 0;
#endif
...
void CP_OptionsMenu(void)
{
	...
	do
	{
		...
		/* R85: HandleMenu dispatches all four surviving rows through
...
{
	while ((*source != 0) && (isspace((unsigned char)*source)) && (length > 0))
	{
		*dest = *source;
		length--;
		dest++;
...

{
	...
	while (*error != 0)
	{
		...
		pos = 0;
		while ((wordtext[pos] != 0) && (isspace((unsigned char)wordtext[pos])))
		{
			pos++;
		}
