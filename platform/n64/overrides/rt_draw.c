...
void DoLoadGameSequence(void)
{
	...
	/* ROTT64_R124_N64_LOAD_TRANSITION_ONE_SCREEN_SCRATCH:
	 * VL_CopyPlanarPageToMemory() copies exactly screensize bytes, where
	 * screensize is width * height. The legacy 64000*8 allocation wastes
	 * 448 KiB at the N64's fixed 320x200 software-render size and exhausted
	 * the zone heap during load. Preserve legacy behavior off N64. */
	#ifdef __N64__
	destscreen = SafeMalloc((long)screensize);
	#else
