...
{
    /* ROTT64_R102_CRT_SAFE_VIEWPORT_SYNC
     *
     ...
     */
    enum {
        ROTT64_R102_SRC_W = 320,
        ROTT64_R102_SRC_H = 200,
        ROTT64_R102_DST_W = 320,
        ROTT64_R102_DST_H = 240,
        ROTT64_R102_SAFE_X = 16,
        ROTT64_R102_SAFE_Y = 12,
        ROTT64_R102_SAFE_W = 288,
        ROTT64_R102_SAFE_H = 216
    };
    unsigned char *base;
    int y;
    uint16_t source_row[ROTT64_R102_SRC_W];

    if (fb == NULL || fb->buffer == NULL)
        return;
    if (fb->width != ROTT64_R102_DST_W || fb->height != ROTT64_R102_DST_H || fb->stride == 0)
        return;
#if defined(__N64__)
    ...
    for (y = ROTT64_R102_DST_H - 1; y >= 0; --y)
    {
        uint16_t *dst = (uint16_t *)(void *)(base + ((size_t)y * fb->stride));
        int x;

        if (y < ROTT64_R102_SAFE_Y || y >= ROTT64_R102_SAFE_Y + ROTT64_R102_SAFE_H)
        {
            for (x = 0; x < ROTT64_R102_DST_W; ++x)
                dst[x] = 0;
            continue;
        }

        {
            const int local_y = y - ROTT64_R102_SAFE_Y;
            const int source_y = (local_y * ROTT64_R102_SRC_H) / ROTT64_R102_SAFE_H;
            const uint16_t *src =
        ...
        for (x = 0; x < ROTT64_R102_SAFE_W; ++x)
        {
            const int source_x = (x * ROTT64_R102_SRC_W) / ROTT64_R102_SAFE_W;
            dst[ROTT64_R102_SAFE_X + x] = source_row[source_x];
        }
    }
}
