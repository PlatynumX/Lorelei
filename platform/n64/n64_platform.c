...
static bool boot_display_open = false;
#endif
...
{
    if (!boot_display_ready) return;
    /* Keep display alive for Taradino handoff. */
...
void n64_platform_init(void)
{
    ...
    boot_display_show("Stage 4/4: Dark War WAD found\nStarting Taradino...");
    wait_ms(1500);
    boot_/* Keep display alive for Taradino handoff. */
#endif
    initialized = true;
}
