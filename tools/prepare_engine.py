...
def prepare(root: Path, upstream: Path, output: Path) -> None:
    ...
    # GCC 14's VR4300 optimized build reports false-positive uninitialized
    # diagnostics in several legacy full-version Taradino renderer/actor paths.
    # Keep the project-wide -Werror policy intact and suppress only these two
    # diagnostics inside generated upstream engine translation units.
    legacy_warning_guard = (
        '#if defined(__GNUC__)\n'
        '#pragma GCC diagnostic ignored "-Wmaybe-uninitialized"\n'
        '#pragma GCC diagnostic ignored "-Wuninitialized"\n'
        '#endif\n'
    )
    for legacy_c in output.glob("*.c"):
        legacy_text = legacy_c.read_text(encoding="utf-8", errors="strict")
        if "pragma GCC diagnostic ignored \"-Wmaybe-uninitialized\"" not in legacy_text:
            legacy_c.write_text(
                legacy_warning_guard + legacy_text,
                encoding="utf-8",
            )
