...
MARK = "ROTT64_R96C_START_INGAME_SAVE_CONTEXT"
...
def patch_menu(gen: Path) -> None:
    ...
    helper_name = "rott64_r96c_can_save_in_current_context"
    ...
    func = text[a:b]
    if "ROTT64_R96C_SAVE_CONTEXT_GUARD" not in func:
        insert = (
            "{\n"
    ...
    if "ROTT64_R96C_SAVE_CONTEXT_GUARD" not in out:
...
def patch_start_sources(gen: Path) -> None:
    ...
    for path in sorted(roots):
        ...
        text = re.sub(
            ...
            r'\1/* ROTT64_R96C_START_NO_TITLE_JUMP: Start must use in-game ControlPanel/Escape path. */',
            text,
        )
...
def main(argv: list[str]) -> int:
    if len(argv) != 2:
        fail("usage: r96c_patch_start_save_context.py generated/rott")
    gen = Path(argv[1])
    if not gen.is_dir():
