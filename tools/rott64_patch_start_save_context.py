...
MARK = "ROTT64_R94F_START_INGAME_SAVE_CONTEXT"
...
def patch_menu(gen: Path) -> None:
    ...
    helper_name = "rott64_r94f_can_save_in_current_context"
    ...
    func = text[a:b]
    if "ROTT64_R94F_SAVE_CONTEXT_GUARD" not in func:
        insert = (
            "{\n"
            "    /* ROTT64_R94F_SAVE_CONTEXT_GUARD */\n"
            f"    if (!{helper_name}())\n"
            "    {\n"
            "        printf(\"ROTT64 r94f: Save rejected outside active game context\\n\");\n"
            "        return 0;\n"
            "    }\n"
        )
    ...
    if "ROTT64_R94F_SAVE_CONTEXT_GUARD" not in out:
...
def patch_start_sources(gen: Path) -> None:
    ...
    for path in sorted(roots):
        ...
        text = re.sub(
            ...
            r'\1/* ROTT64_R94F_START_NO_TITLE_JUMP: Start must use in-game ControlPanel/Escape path. */',
            text,
        )
...
def main(argv: list[str]) -> int:
    if len(argv) != 2:
        fail("usage: r94f_patch_start_save_context.py generated/rott")
    gen = Path(argv[1])
    if not gen.is_dir():
