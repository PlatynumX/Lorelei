...
MARK = "ROTT64_R94C_ATOMIC_SAVE_VALIDATE"
...
def patch_rt_game(gen: Path) -> None:
    ...
    helper = r'''
/* ROTT64_R94C_ATOMIC_SAVE_VALIDATE: N64-safe save probing/commit.
...
'''
    ...
    load_func = text[a:b]
    if "ROTT64_R94C_LOAD_VALIDATE_BEFORE_FULL_LOAD" not in load_func:
        ...
        guard = r'''
    /* ROTT64_R94C_LOAD_VALIDATE_BEFORE_FULL_LOAD */
...
'''
    ...
    save_func = text[a:b]
    if "ROTT64_R94C_SAVE_TEMP_PATH" not in save_func:
        ...
        new = ("/* ROTT64_R94C_SAVE_TEMP_PATH */\n"
...
{indent}{{
{indent}{indent}printf("ROTT64 r94c: temp save failed validation, deleting %s\n", filename);
...
{indent}{{
{indent}{indent}printf("ROTT64 r94c: failed to commit save %s -> %s\n", filename, finalname);
{indent}{indent}remove(filename);
{indent}{indent}free(filename);
{indent}{indent}free(finalname);
{indent}{indent}return false;
...
{indent}return (true);'''
        ...
        if "char *finalname" not in save_func or "ROTT64_R94C_SAVE_TEMP_PATH" not in save_func:
...
def patch_rt_menu(gen: Path) -> None:
    ...
    /* ROTT64_R94C_SCAN_VALIDATES_SLOTS */
    ...
    # ROTT64_R94C_NO_QUICKSAVE_ELSE_SPLICE:
    ...
    out = path.read_text(encoding="utf-8", errors="ignore")
    if "ROTT64_R94C_SCAN_VALIDATES_SLOTS" not in out:
...
def main(argv: list[str]) -> int:
    if len(argv) != 2:
        fail("usage: r94c_patch_atomic_save_validate.py generated/rott")
    gen = Path(argv[1])
    if not gen.is_dir():
