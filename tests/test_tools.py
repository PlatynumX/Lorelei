...
def test_n64_warning_policy() -> None:
    ...
    # Dark War compiles additional legacy Taradino paths that trigger
    # maybe-uninitialized/uninitialized under the current GCC. Keep -Werror
    # globally, but allow only these two warning classes to remain non-fatal.
    assert "-Werror" in makefile
    assert "-Wno-error=maybe-uninitialized" in makefile
    assert "-Wno-error=uninitialized" in makefile

    scrubbed = makefile.replace("-Wno-error=maybe-uninitialized", "")
    scrubbed = scrubbed.replace("-Wno-error=uninitialized", "")
    assert "-Wno-error" not in scrubbed
