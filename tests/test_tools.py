...
def test_no_local_split_commbat_hooks() -> None:
    platform = (ROOT / "platform/n64/n64_platform.c").read_text(encoding="utf-8")
    sdl = (ROOT / "platform/n64/sdl_n64.c").read_text(encoding="utf-8")
    modex = (ROOT / "platform/n64/modexlib_n64.c").read_text(encoding="utf-8")
    prepare = (ROOT / "tools/prepare_engine.py").read_text(encoding="utf-8")
    combined = "\n".join((platform, sdl, modex, prepare))
    assert "n64_platform_split_commbat_requested" not in combined
    assert "split_view_framebuffer" not in combined
    assert "ROTT64_PollControlsSingle" not in combined
    assert "ROTT64_ThreeDRefreshSingle" not in combined
    assert "numplayers = 2" not in combined
