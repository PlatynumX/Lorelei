...
def test_n64_warning_policy() -> None:
    ...
    # Keep libdragon's strict build policy. Legacy full-version false positives
    # are handled only inside generated Taradino translation units.
    assert "-Wno-error" not in makefile
    assert 'pragma GCC diagnostic ignored "-Wmaybe-uninitialized"' in prepare
    assert 'pragma GCC diagnostic ignored "-Wuninitialized"' in prepare
