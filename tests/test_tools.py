...
def test_n64_warning_policy() -> None:
    ...
    # Global/platform builds stay strict. Only generated upstream Taradino
    # engine objects may demote warnings from fatal errors.
    assert "$(BUILD_DIR)/generated/rott/%.o: CFLAGS += -Wno-error" in makefile

    scrubbed = makefile.replace(
        "$(BUILD_DIR)/generated/rott/%.o: CFLAGS += -Wno-error", ""
    )
    assert "CFLAGS += -Wno-error" not in scrubbed
